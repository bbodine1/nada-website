	...
	useEffect(() => {
		...
		const handlePopState = (event: PopStateEvent) => {
			...
			const hasSentinel = Boolean(
				(state as Record<string, unknown>)[MOBILE_SENTINEL_KEY],
			);
			...
			window.history.pushState(
				{ ...currentState, [MOBILE_SENTINEL_KEY]: true },
				"",
			);
			mobileSentinelActiveRef.current = true;
		};
	...
	async function handleSubmit(event: FormEvent<HTMLFormElement>) {
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					data.error ?? "Could not submit the form. Please try again.",
				);
			}
				...
				{formState === "success" ? (
					<div className="relative">
						...
						<p className="mt-2 text-sm text-[color:var(--fg-muted)]">
							Download the field guide now—we&apos;ll email the same link.
						</p>
						<a
							href={PDF_PATH}
							download
					...
					>
						<div>
							<p
								...
								className="font-heading text-2xl font-semibold text-[color:var(--color-primary)]"
							>
								Before you go—get the free field guide.
							</p>
							<p className="mt-2 text-sm text-[color:var(--fg-muted)]">
								First name and email only. See when drones save time and yield
								on your acres—and when they don&apos;t.
							</p>
						</div>
